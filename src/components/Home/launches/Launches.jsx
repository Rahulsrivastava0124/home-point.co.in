...
export default function Launches() {
  ...
  return (
    <div
      className="md:py-26 py-16 md:mx-10  bg-[#571078] md:rounded-2xl"
      id="launches"
    >
      <h1 className="capitalize heading_gradient md:text-5xl text-3xl w-fit mx-auto relative">
        {" "}
        Fresh new launches{" "}
      </h1>
      <h1 className="text-center mt-1 text-white md:text-[15px] text-xs ">
      ...
      <div className="mt-10 md:w-10/12  mx-auto relative">
        {isLoading ? (
          ...
          <div className="text-center py-16">
            <h2 className="text-3xl md:text-4xl font-bold text-white august">
              Coming Soon
            </h2>
            <p className="text-gray-300 mt-2">
              New launches will be available soon!
            </p>
          </div>
        ) : (
          <Swiper
          ...
          >
            {launches.map((item, index) => (
              <SwiperSlide key={index} className="pb-10">
                <Link
                  to={`/projects/${item.title.replace(/\s+/g, "_").toLowerCase()}`}
                  state={{ project: item.originalData }}
                >
